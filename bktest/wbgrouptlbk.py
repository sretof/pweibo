...
def getmaxgpmids():
    ...
    try:
        results = coll.aggregate([{'$match': {'fday': {'$gt': '20190701'}}}, {'$group': {'_id': "$gid", 'maxmid': {'$max': "$mid"}}}])
        gsmid = {}
        for res in results:
...
def updateMid(umweibo):
    ...
    for doc in result:
        ...
        if 'smid' in doc:
            ...
            continue
        if isinstance(mid, int):
            nmid = PWeiBo.directfpage(umweibo, cturl)
            if not nmid:
                nmid = mid
                if nmid < 0:
                    nmid = nmid * -1
                nmid = str(nmid)
            udpgpdetail(mid, 'smid', cday + str(nmid))
            udpgpdetail(mid, 'mid', nmid)
        else:
            nmid = mid
            udpgpdetail(mid, 'smid', cday + str(nmid))
...
if __name__ == '__main__':
    username = 'sretof@live.cn'
    password = '1122aaa'
    mpweibo = PWeiBo(username, password)
    mpweibo.login()
    print(mpweibo.pwuid)
    # while 1:
    #     # 登录
