...
if __name__ == '__main__':
    ...
    wbcomp = WbComp(dbc.WBUN, dbc.WBPW, mlogger=tllogger)
    wbcomp.login()
    utlwbcomp = WbComp(dbc.WBUN, dbc.WBPW, mlogger=utllogger)
    utlwbcomp.login()
    pagecmp = WbPageCmp(wbcomp, mlogger=tllogger)
    gtlcmp = WbGTlCmp(wbcomp, pagecmp=pagecmp, mlogger=tllogger)
    ...
    fwdcmp = WbFwdCmp(wbcomp, utlcmp, mlogger=flogger)
    fwdcmp.ffwddocsex()
    while 1:
        gtlcmp.fgroupstl()
        fgtlslp(tllogger)
